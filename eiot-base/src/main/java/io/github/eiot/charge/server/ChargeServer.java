 ...
 * <p>
 * You receive {@link ChargeConnection} by providing a {@link #connectionHandler()}. As charge connection arrive on the server, the
...
public interface ChargeServer {
     ...
     * {@link ChargeConnection} that was used when creating the server.
     * <p>
     * The listen happens asynchronously and the server may not be listening until some time after the call has returned.
     *
     * @return a future completed with the listen operation result
     */
    Future<ChargeServer> listen();

    /**
     * Like {@link #listen(int, String)} but the server will listen on host "0.0.0.0" and port specified here ignoring
     * any value in the {@link ChargeServerOptions} that was used when creating the server.
     *
     * @param port the port to listen on
     * @return a future completed with the listen operation result
     */
    default Future<ChargeServer> listen(int port) {
        return listen(port, "0.0.0.0");
    }

    /**
     * Tell the server to start listening. The server will listen on the port and host specified here,
     * ignoring any value set in the {@link ChargeServerOptions} that was used when creating the server.
     * <p>
     * The listen happens asynchronously and the server may not be listening until some time after the call has returned.
     *
     * @param port the port to listen on
     ...
     * @return a future completed with the listen operation result
     */
    default Future<ChargeServer> listen(int port, String host) {
     ...
     */
    Future<ChargeServer> listen(SocketAddress address);
     ...
     */
    ChargeServer frameHandler(Handler<Frame<?>> handler);
     ...
     * @return a reference to this, so the API can be used fluently
     */
    ChargeServer connectionHandler(Handler<ChargeConnection> handler);

    /**
     * @return the connection handler
     */
    Handler<ChargeConnection> connectionHandler();
     ...
     * @return a reference to this, so the API can be used fluently
     */
    ChargeServer exceptionHandler(Handler<Throwable> handler);
