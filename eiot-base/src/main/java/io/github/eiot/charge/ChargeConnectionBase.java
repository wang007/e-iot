...
public abstract class ChargeConnectionBase extends ConnectionBase implements ChargeConnection {
    ...
    protected ChargeConnectionBase(ContextInternal context, ChannelHandlerContext chctx,
        ...
        closeFuture().onComplete(ar -> {
            synchronized (ChargeConnectionBase.this) {
                this.closed = true;
            }
        });
    ...
    @Override
    public synchronized ChargeConnectionBase exceptionHandler(Handler<Throwable> handler) {
        super.exceptionHandler(handler);
        return this;
    }

    @Override
    public synchronized ChargeConnection frameHandler(Handler<Frame<?>> handler) {
        this.frameHandler = handler;
        return this;
    }

    @Override
    public synchronized ChargeConnection sendWriteHook(SendWriteHook hook) {
    ...
    @Override
    public synchronized ChargeConnection handler(@Nullable Handler<Buffer> handler) {
        this.handler = handler;
        return this;
    }

    @Override
    public ChargeConnection pause() {
        pending.pause();
        return this;
    }

    @Override
    public ChargeConnection resume() {
        pending.resume();
        return this;
    }

    @Override
    public ChargeConnection fetch(long amount) {
        pending.fetch(amount);
        return this;
    }

    @Override
    public synchronized ChargeConnection endHandler(@Nullable Handler<Void> endHandler) {
    ...
    @Override
    public ChargeConnection setWriteQueueMaxSize(int maxSize) {
    ...
    @Override
    public synchronized ChargeConnection drainHandler(@Nullable Handler<Void> handler) {
